<template>
	<View class="SongList">
		<View class="header">
			<Image :src="require('../../static/play-cell.png')" @click="playAll" />
			<View class="left" @click="playAll">
				播放全部<Text>(共{{ SongList.length }}首)</Text>
			</View>
			<View class="right">评分星级</View>
		</View>
		<View class="item" v-for="(item, index) in SongList" :key="index" @click="go(item)">
			...
			<View class="box">
				...
				<View class="right">
					<van-rate readonly color="#ee0a24" icon="like" void-icon="like-o" :size="rateConfig.size" :gutter="rateConfig.gutter"
					 :value="item.value" />
				</View>
			</View>
		</View>
...
<script>
	...
	import VanRate from "../../wxcomponents/vant-weapp/rate/index";
	...
	export default {
		...
		onLoad() {
			...
			getRating({
				...
				console.log('获取评分返回结果：', res)
				if (res['code'] == '200') {
					// 关闭加载动画
					uni.hideLoading();
					for (let item of res.ratings) {
						this.SongList.push({
							al: {
