	...
	<View class="Song">
		...
		<View class="img-box">
			<Image :class="['rotate', audio_paused ? 'paused' : '']" mode="widthFix" :src="
                    song.album_picture
                        ? song.album_picture + '?imageView&thumbnail=480x0'
                        : require('../../static/defaultMusicAvatar.jpg')
                " />
		</View>
		...
		<View class="footer">
			...
			<View class="Lyric">
				<view class="rate-content">
					<uni-rate active-color="#ee0a24" size="18" margin='10' :value="ratingVal" @change="onChange"></uni-rate>
				</view>
			</View>
			...
			<View class="range-box">
				...
				<slider :value="sliderValue" activeColor="#C30000" backgroundColor="rgba(255,255,255,0.4)" blockColor="#C30000"
...
<script>
	...
	import uniRate from '@/components/wyc-rate/uni-rate.vue';
	import {
		getOneRating,
		addRating,
		getPlayingInfo
	...
	export default {
		...
		onLoad(param) {
			console.log("页面传值获取到的id：", param.id)
			this.songid = param.id
			// 判断用户是否登录
			if (getApp().globalData.LOGIN_STATUS) {
				// 获取评分值
				getOneRating({
					user_id: getApp().globalData.USER_INFO.USER_ID,
					track_id: this.songid //页面传过来的参数
				}).then(res => {
					console.log("获取某首歌的评分值：", res)
					if (res.code == 200) {
		...
		created() {
			...
			console.log("created生命周期中获取的songData:", songData)

			if (songData.id !== audio.id) {
				//矫正当前播放的src
				songData.src = baseUrl.serverUrl + audio.track_data_url;
				console.log("当前播放的url:", songData.src)
				this.$store.dispatch("setGlobalData", {
					key: "song",
					value: songData
				});
		...
		methods: {
			 ...
			 */
			updateStore() {
				console.log("是否暂停：", this.$store.state.audio.paused);
				this.audio_name = this.$store.state.audio._singer;
				this.audio_singer = this.$store.state.audio._singer;
